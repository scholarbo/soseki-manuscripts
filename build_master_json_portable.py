...
MASTER_REQUIRED_COMMON = ['seq', 'title', 'source', 'date_year', 'date_month', 'date_day']
...
def work_id_from_row(row: dict) -> str:
    return clean(row.get('work_id') or row.get('id'))
...
def main():
    ...
    ensure_headers(master_headers, MASTER_REQUIRED_COMMON, 'master.xlsx')
    if 'work_id' not in master_headers and 'id' not in master_headers:
        raise ValueError('master.xlsx 缺少必要列: work_id 或 id')
    ...
    for row in images_rows:
        ...
        if not work_id or not manifest_id:
            continue
        entry = {'witness_id': witness_id, 'manifest_id': manifest_id}
        if entry not in by_work[work_id]:
            by_work[work_id].append(entry)

    result = []
    for row in master_rows:
        work_id = work_id_from_row(row)
        if not work_id:
            continue

        manifests = sorted(by_work.get(work_id, []), key=lambda x: (x['witness_id'], x['manifest_id']))
        manifest_list = []
        witness_list = []
        ...
        default_witness = clean(row.get('default_witness')) or (manifest_list[0]['witness_id'] if manifest_list else '')
        default_manifest = clean(row.get('default_manifest'))
        if not default_manifest and manifest_list:
            matched = next((m['manifest_url'] for m in manifest_list if m['witness_id'] == default_witness), '')
            default_manifest = matched or manifest_list[0]['manifest_url']
            if not default_witness:
                default_witness = manifest_list[0]['witness_id']

        out = {k: clean(v) for k, v in row.items() if clean(k) != 'manifest'}
        out['work_id'] = work_id
        out['id'] = work_id
        ...
        out['witness_count'] = len(witness_list)
        out['has_multiple_witness'] = '1' if len(witness_list) > 1 else '0'
        result.append(out)

    result.sort(key=lambda x: (int(x.get('seq') or 999999), x.get('work_id') or x.get('id') or ''))
